    ...
    <p>
        <asp:GridView ID="GridView1" runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False" DataKeyNames="ID" DataSourceID="StaffDetails">
            <Columns>
                ...
                <asp:CheckBoxField DataField="ResearchLeave" HeaderText="ResearchLeave" SortExpression="ResearchLeave" />
                <asp:CommandField ShowDeleteButton="True" ShowEditButton="True" />
            </Columns>
        </asp:GridView>
        <asp:SqlDataSource ID="StaffDetails" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:staffwoAJ4TKlNRsConnectionString %>" DeleteCommand="DELETE FROM [StaffDetails] WHERE [ID] = @original_ID AND [Name] = @original_Name AND [MaxHours] = @original_MaxHours AND (([CurrentHours] = @original_CurrentHours) OR ([CurrentHours] IS NULL AND @original_CurrentHours IS NULL)) AND (([InitialYear] = @original_InitialYear) OR ([InitialYear] IS NULL AND @original_InitialYear IS NULL)) AND (([ResearchLeave] = @original_ResearchLeave) OR ([ResearchLeave] IS NULL AND @original_ResearchLeave IS NULL))" InsertCommand="INSERT INTO [StaffDetails] ([Name], [MaxHours], [CurrentHours], [InitialYear], [ResearchLeave]) VALUES (@Name, @MaxHours, @CurrentHours, @InitialYear, @ResearchLeave)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT * FROM [StaffDetails]" UpdateCommand="UPDATE [StaffDetails] SET [Name] = @Name, [MaxHours] = @MaxHours, [CurrentHours] = @CurrentHours, [InitialYear] = @InitialYear, [ResearchLeave] = @ResearchLeave WHERE [ID] = @original_ID AND [Name] = @original_Name AND [MaxHours] = @original_MaxHours AND (([CurrentHours] = @original_CurrentHours) OR ([CurrentHours] IS NULL AND @original_CurrentHours IS NULL)) AND (([InitialYear] = @original_InitialYear) OR ([InitialYear] IS NULL AND @original_InitialYear IS NULL)) AND (([ResearchLeave] = @original_ResearchLeave) OR ([ResearchLeave] IS NULL AND @original_ResearchLeave IS NULL))">
            <DeleteParameters>
